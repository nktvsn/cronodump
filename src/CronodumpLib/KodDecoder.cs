...
{
    /// <summary>
    /// Implements the KOD encoding used by CroStru files.
    /// </summary>
    public class KODcoding
    {
        public static readonly byte[] INITIAL_KOD = new byte[] {
    ...
    0xFC,0x84,0xE5,0xF9,0x14,0x19,0xDF,0x6E,0x23,0xC4,0x66,0xEB,0xCC,0x22,0x1C,0x5C,
        ...
        private readonly byte[] kod;
        private readonly byte[] inv;

        public KODcoding(byte[]? initial = null)
        {
            kod = initial != null ? (byte[])initial.Clone() : INITIAL_KOD.ToArray();
            inv = new byte[256];
            for (int i = 0; i < kod.Length; i++)
                inv[kod[i]] = (byte)i;
        }

        public byte[] Decode(int shift, byte[] data)
        {
            byte[] res = new byte[data.Length];
            for (int i = 0; i < data.Length; i++)
                res[i] = (byte)((kod[data[i]] - i - shift) & 0xFF);
            return res;
        }

        public byte[] Encode(int shift, byte[] data)
        {
            byte[] res = new byte[data.Length];
            for (int i = 0; i < data.Length; i++)
                res[i] = inv[(data[i] + i + shift) & 0xFF];
            return res;
        }

        public static KODcoding New() => new KODcoding();
        public static KODcoding New(byte[] table) => new KODcoding(table);
    }
}
